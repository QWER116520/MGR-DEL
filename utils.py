...
def data2gpu(batch, use_cuda, data_type):
    if use_cuda:
        if data_type == 'rationale':
            # 新数据集顺序: content, content_masks,
            # FTR_2_pred, FTR_2_acc, FTR_3_pred, FTR_3_acc,
            # FTR_2, FTR_2_masks, FTR_3, FTR_3_masks,
            # RP_token_ids, RP_masks, pixel_values, label, id, category, image_text_relevance
            batch_size = batch[0].size(0)
            device = batch[0].device
            
            # 检查batch长度，确保image_text_relevance存在
            if len(batch) >= 17:
                image_text_relevance_tensor = batch[16].cuda()
            else:
                # 如果batch长度不足，创建默认的image_text_relevance tensor（全0，表示'real'）
                print(f"⚠️ 警告: batch长度不足 ({len(batch)} < 17)，使用默认image_text_relevance")
                image_text_relevance_tensor = torch.zeros(batch_size, dtype=torch.long, device=device)
            ...
            batch_data = {
                ...
                'RP_token_ids': batch[10].cuda(),
                'RP_masks': batch[11].cuda(),
                'pixel_values': batch[12].cuda(),
                'label': batch[13].cuda(),
                'id': batch[14].cuda(),
                'category': batch[15].cuda(),
                'image_text_relevance': image_text_relevance_tensor,  # ✅ 新增 image_text_relevance
            }
        else:
        ...
        if data_type == 'rationale':
            ...
            # 检查batch长度，确保image_text_relevance存在
            if len(batch) >= 17:
                image_text_relevance_tensor = batch[16]
            else:
                # 如果batch长度不足，创建默认的image_text_relevance tensor（全0，表示'real'）
                print(f"⚠️ 警告: batch长度不足 ({len(batch)} < 17)，使用默认image_text_relevance")
                image_text_relevance_tensor = torch.zeros(batch_size, dtype=torch.long)
            ...
            batch_data = {
                ...
                'FTR_3': batch[8],
                'FTR_3_masks': batch[9],
                'RP_token_ids': batch[10],
                'RP_masks': batch[11],
                'pixel_values': batch[12],
                'label': batch[13],
                'id': batch[14],
                'category': batch[15],
                'image_text_relevance': image_text_relevance_tensor,  # ✅ 新增 image_text_relevance
...
def get_tensorboard_writer(config):
    ...
    writer = SummaryWriter(log_dir=writer_dir, flush_secs=5)
